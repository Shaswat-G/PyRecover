...
def parse_args():
    parser = argparse.ArgumentParser(description="Check if model weights are equal between two checkpoints")
    ...
    parser.add_argument("--distributed", action="store_true", help="Use distributed checkpoint loading functions")
    parser.add_argument("--tolerance", type=float, default=1e-7, help="Tolerance for weight comparison")
    parser.add_argument("--verbose", action="store_true", help="Print detailed comparison results")
    return parser.parse_args()


def extract_model_state_dict(checkpoint_path: str, distributed: bool) -> Dict[str, torch.Tensor]:
    """
...
def compare_weights(
    ...
    for key in keys1:
        ...
        if max_diff_param > tolerance:
            if verbose:
                print(f"❌ Parameter '{key}' has difference {max_diff_param:.8e} > tolerance {tolerance:.8e}")
            all_equal = False
            diff_param_count += 1
    
    # Report results
    if all_equal:
        print(f"✅ All {total_params} parameters are equal within tolerance {tolerance}")
        print(f"Maximum difference: {max_diff:.8e}")
    else:
        print(f"❌ {diff_param_count} out of {total_params} parameters differ beyond tolerance {tolerance}")
        print(f"Maximum difference: {max_diff:.8e}")
...
def main():
    ...
    print(f"Using {'distributed' if args.distributed else 'vanilla'} checkpoint loading")
