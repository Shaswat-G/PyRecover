#!/bin/bash
#SBATCH --job-name=timeaware_dist_ckpt_30m
...
#SBATCH --output=/iopsstor/scratch/cscs/%u/timeaware_dist_ckpt_30m_%j.out
#SBATCH --error=/iopsstor/scratch/cscs/%u/timeaware_dist_ckpt_30m_%j.err
...
python3 train.py --distributed --use-torch-distributed-ckpt --timeaware-checkpointing --log-loss-to-csv --experiment_name=timeaware_dist_ckpt_30m --checkpoint-frequency $CHECKPOINT_FREQ --training-steps $TRAINING_STEPS --logging-frequency $LOGGING_FREQ --batch-size=$GLOBAL_BATCH_SIZE --default-iter-time=$ITER_TIME --default-ckpt-time=$CKPT_TIME
"
