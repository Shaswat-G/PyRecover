...
def save_ckpt_vanilla(
    ...
    if rank == 0 or not is_distributed:
        ...
        if max_keep > 0:
            ...
            ckpt_files = [
                x.relative_to(ckpt_base_path)
                for x in ckpt_base_path.glob("**/*.pt")
                if x.is_file()
            ]
            ckpt_files.sort()
...
def load_ckpt_distributed(
    ...
    print(
        f"Distributed checkpoint loaded from {checkpoint_path} (epoch {epoch}, step {step})"
    )
    return epoch, step


def get_latest_checkpoint(
    checkpoint_dir: str, distributed: bool = False
) -> Optional[str]:
    """
